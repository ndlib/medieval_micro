...
#
# TODO: extend, rather than override, this module
#
module BlacklightHelper
  ...
  def render_show_field_label args
    field = args[:field]
  ...
  def render_document_show_field_value args
    value = args[:value] || document_show_field_value(args)
    render_field_value value
  end

  def document_show_field_value args
    value ||= args[:document].get(args[:field], :sep => nil) if args[:document] and args[:field]
  ...
  def link_to_document(doc, opts={:label=>Blacklight.config[:index][:show_link].to_sym, :counter => nil, :results_view => true})
    ...
    label_with_help = label + "<span class=\"help\">View full Record</span>"
    link_to_with_data(label_with_help.html_safe , catalog_path(doc[:id]), {:method => :put, :class => label.parameterize, :data => opts}).html_safe
  end
  ...
  # This is an updated +link_to+ that allows you to pass a +data+ hash along with the +html_option
  ...
  def render_document_as_per_format!(document)
    if is_microfilm?(document)
      document.delete('title_display')
    end
    return document
  end

  def is_microfilm?(document)
    if document['format'] == "Microfilm"
      return true
    end
    false
  end
